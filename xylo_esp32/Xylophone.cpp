...
------------------------------    XYLOPHONE.CPP (ESP32 VERSION)   --------------------------------------
_________________________________________________________________________________________________________

Class to manage xylophone actions - ESP32 adaptation
...
Xylophone::Xylophone()
  ...
  // Initialize ESP32 Ticker - attach callback with 5ms interval
  _electromagnetTicker.attach_ms(5, _electromagnetTickerCallback);
}
...
bool Xylophone::begin() {
  ...
  if (DEBUG_XYLO) {
    Serial.println(F("PWM configured: channel="));
    Serial.print(PWM_CHANNEL);
    Serial.print(F(", freq="));
    Serial.print(PWM_FREQUENCY);
    Serial.print(F("Hz, resolution="));
...
void Xylophone::playNote(byte note, byte velocity) {
  ...
  noInterrupts();
  {
    ...
    // Ticker is already running continuously on ESP32
    // No need to start/stop (always active)
 ...
 * @note On ESP32, Ticker runs automatically via interrupt, no update needed
 */
void Xylophone::update() {
  // ESP32 Ticker runs automatically in background, no manual update required
}
...
void Xylophone::stopNote(byte midiNote) {
  ...
  noInterrupts();
  {
    ...
    // Turn off PWM when no notes playing
    if (_electromagnetCount == 0) {
      // Ticker continues running on ESP32 (low overhead)
      ledcWrite(PWM_CHANNEL, PWM_OFF_VALUE);  // Turn off PWM
    }
  }
  interrupts();  // Re-enable interrupts
